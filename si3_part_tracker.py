...
if __name__ == '__main__':
    ...
    #
    if lvlist:
        xSclV  = np.zeros((Nt+1,nP,NeV)) + sit.FillValue ; # to store value of specified scalars at the buoy position
    ...
    for jt in range(Nt):
        ...
        for jP in range(nP):            
            ...
            if iAlive[jP]==1 and jrec>=z1stModelRec[jP] and jrec<=zLstModelRec[jP]:
                ...
                [ ry  , rx   ] = xPosC[jt,jP,:] ; # position (Cartesian coord.)  of current buoy on the target (projected domain) ! [km]
                [ rlat, rlon ] = xPosG[jt,jP,:] ; # position (geographic coord.) of current buoy
    ...
    if not lUse2DTime:
        ...
        if lvlist:
            kk = sit.ncSaveCloudBuoys( cf_nc_out, vTime, IDs, xPosG[:,:,0], xPosG[:,:,1], pY=xPosC[:,:,0], pX=xPosC[:,:,1],
                                       mask=xmask[:,:,0], corigin=corgn, Scalars=xSclV[:,:,:], listSclr=vlist )
        else:
            kk = sit.ncSaveCloudBuoys( cf_nc_out, vTime, IDs, xPosG[:,:,0], xPosG[:,:,1], pY=xPosC[:,:,0], pX=xPosC[:,:,1],
